...
if (($this->input->get('broughtfwd') != '')&&($this->input->get('req') != '10')){
   $tajuk = 'Unscheduled Brought Forward Work Order Details';
}elseif(($this->input->get('broughtfwd') != '')&&($this->input->get('req') == '10')){
  $tajuk = 'Work Order A10 Summary Report';
}
else{
...
<div class="none">
	<?php if ($this->input->get('ex') == ''){?>
		...
		<div id="Instruction" class="pr-printer">
    		<div class="header-pr"><?php if ($this->input->get('broughtfwd') != ''){?>Unscheduled Brought Forward Work Order Details<?php }else{ ?>Work Order Listing Details Report<?php }?></div>
    		<button onclick="javascript:myFunction('wrkorder_report?m=<?=$month?>&y=<?=$year?>&ex=&stat=<?=$this->input->get('stat');?>&grp=<?=$this->input->get('grp');?>&wid=1&req=<?=$this->input->get('req');?>&broughtfwd=<?=$this->input->get('broughtfwd')?>&serv=<?=$this->input->get('serv')?>&tag=<?=$this->input->get('tag')?>&cm=<?=$this->input->get('cm')?>&none=closed');" class="btn-button btn-primary-button">PRINT</button>
    		<button type="cancel" class="btn-button btn-primary-button" onclick="location.href = '<?php echo $btp ;?>';">CANCEL</button>
			<?php if (($this->input->get('ex') == '') or ($this->input->get('none') == '')){?>
				<a href="<?php echo base_url();?>index.php/contentcontroller/wrkorder_report?req=<?=$this->input->get('req');?>&m=<?=$this->input->get('m');?>&y=<?=$this->input->get('y');?>&ex=excel&none=close&stat=<?=$this->input->get('stat');?>&resch=<?=$this->input->get('resch');?>&broughtfwd=<?=$this->input->get('broughtfwd');?>&xxx=export&grp=<?=$this->input->get('grp');?>&serv=<?=$this->input->get('serv')?>>&tag=<?=$this->input->get('tag')?>&cm=<?=$this->input->get('cm')?>" style="float:right; margin-right:40px;"><img src="<?php echo base_url();?>images/excel.png" style="width:40px; height:38px; position:absolute;" title="export to excel"></a>
				<a href="<?php echo base_url();?>index.php/contentcontroller/wrkorder_report?req=<?=$this->input->get('req');?>&m=<?=$this->input->get('m');?>&y=<?=$this->input->get('y');?>&ex=excel&none=close&stat=<?=$this->input->get('stat');?>&resch=<?=$this->input->get('resch');?>&broughtfwd=<?=$this->input->get('broughtfwd');?>&pdf=1&grp=<?=$this->input->get('grp');?>&serv=<?=$this->input->get('serv')?>&tag=<?=$this->input->get('tag')?>&cm=<?=$this->input->get('cm')?>" style="float:right; margin-right:80px;"><img src="<?php echo base_url();?>images/pdf.png" style="width:40px; height:38px; position:absolute;" title="export to pdf"></a>
			<?php } ?>
		</div>
	<?php } ?>
			...
			<div class="m-div">
					...
					<tr>
						<?php if ($this->input->get('req') == $req) {?>
							<td colspan="4" valign="top">
								...
								<?php }else{ ?>
									Work Order Listing Details Report
				...
				<table class="tftable" border="1" style="text-align:center;">
					<tr>
						...
						<th <?=$rowspan?>>Asset Description</th>
					...
					<?php  if (!empty($record)) {?>
						<?php $numrow = 1; foreach($record as $row):?>
							...
							<!--line-->		<td><?= (($row->V_request_type == "A10") || ($row->V_request_type == "A34") || (($row->v_tag_no == $assetone) && ($row->v_location_code == $locationone)) || ($row->linker != "none")) ? '0' : $row->DiffDate ?></td>
								...
								<?php } ?>
								<td><?= ($row->DocReferenceNo) ? $row->DocReferenceNo : 'N/A' ?></td>
								<td><?= ($row->DateCreated) ? date("d/m/Y",strtotime($row->DateCreated)): 'N/A' ?></td>
								<td><?= ($row->PO_No) ? $row->PO_No : 'N/A' ?></td>
								<td><?= ($row->PO_Date) ? date("d/m/Y",strtotime($row->PO_Date)): 'N/A' ?></td>
						...
						<div class="m-div">
							...
							<table class="tftable tbl-go" border="1" style="text-align:center;">
								<tr>
									...
									<th style="width:3%" <?=$rowspan?>>Time Req</th>
									<th style="width:3%" <?=$rowspan?>>Request No</th>
									<th style="width:3%" <?=$rowspan?>>Asset No</th>
									<th style="width:3%" <?=$rowspan?>>Asset Description</th>
									<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>Request Summary</th>
									<th style="width:3%" <?=$rowspan?>>ULC</th>
									<th style="width:3%" <?=$rowspan?>>Requestor<br>Name</th>
									<th style="width:3%" <?=$rowspan?>>Status</th>
									<th style="width:3%" <?=$display?> colspan=2>Test</th>
									<?php if ($this->input->get('stat') == 'A') {?>
									...
									<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:5%;'";}?>>MRIN<br>No</th>
								    <th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:5%;'";}?>>MRIN<br>Date</th>
									<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:5%;'";}?>>PO<br>No</th>
								    <th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:5%;'";}?>>PO<br>Date</th>
							...
							<table class="tftable tbl-go" border="1" style="text-align:center;">
								<tr>
									...
									<?php  } else {?>
										...
										<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:5%;'";}?>>Duration<br>of Repair (Days)</th>
										<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>Respond Finding</th>
										<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>Completion Date</th>
										<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>Completion Time</th>
									<?php } ?>

									<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>Dept/Loc</th>
									<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>MRIN No</th>
									<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>MRIN Date</th>
									<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>PO No</th>
									<th <?=$rowspan?> <?php if($this->input->get('wid')== 1){ echo "style='width:10%;'";}?>>PO Date</th>
									<th <?=$rowspan?> ><?php if($this->input->get('wid')== 1){ echo "grp";}else{ if($this->input->get('broughtfwd') != ''){echo "Work Order Group";} else{echo "Asset Group";}}?></th>
								</tr>
