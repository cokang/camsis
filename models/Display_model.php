...
/*
      ...
      $this->db->order_by("r.d_date, g.v_tag_no");
			...
			if ($resch == "in"){
				//exit();
				$this->db->where('jv.d_reschdt IS NOT NULL');
			    $this->db->where('jv.d_Date <', $this->dater(1,$month,$year));
				$this->db->group_by('jv.v_WrkOrdNo,jv.n_Visit');

			}
			else if($resch == "out"){
				//echo "test";
				$this->db->where('jv.d_reschdt IS NOT NULL');
			    $this->db->where('jv.d_reschdt >=', $this->dater(2,$month,$year));
				$this->db->group_by('jv.v_WrkOrdNo,jv.n_Visit');
			}
...
//			}
        if ($broughtfwd == ''){
          //$this->db->select("g.V_Asset_name, e.v_location_name, r.v_location_code, r.V_hospitalcode, r.closedby, r.D_date, r.D_time, r.V_Request_no, r.V_Asset_no, r.V_summary AS ReqSummary, r.V_User_dept_code, r.V_requestor, r.V_request_status, r.v_closeddate, r.v_closedtime, w.V_Wrn_end_code, a.v_summary, g.v_tag_no, d.v_UserDeptDesc, DATEDIFF(IFNULL(r.v_closeddate,'".$this->dater(3,$month,$year)."'),r.D_date) + 1 AS DiffDate,r.V_request_type,g.v_asset_grp,jr.d_Date,jr.v_Time,jr.v_Personal1,jr.v_ActionTaken,g.V_Asset_WG_code, IFNULL(dt.ori_wo,'none') AS linker,jv.d_Date AS schedule_d, jv.d_Reschdt AS dtresch", false);
          $this->db->select("g.V_Asset_name, e.v_location_name, r.v_location_code, r.V_hospitalcode, r.closedby, r.D_date, r.D_time, r.V_Request_no, r.V_Asset_no, r.V_summary AS ReqSummary, r.V_User_dept_code, r.V_requestor, r.V_request_status, r.v_closeddate, r.v_closedtime, w.V_Wrn_end_code, a.v_summary, g.v_tag_no, d.v_UserDeptDesc, DATEDIFF(IFNULL(r.v_closeddate,'".$this->dater(3,$month,$year)."'),r.D_date) + 1 AS DiffDate,r.V_request_type,g.v_asset_grp,jr.d_Date,jr.v_Time,jr.v_Personal1, IFNULL((select v_ActionTaken from pmis2_emg_jobvisit1 where v_WrkOrdNo = r.V_Request_no order by n_Visit desc limit 1),r.v_summary) as v_ActionTaken,g.V_Asset_WG_code, IFNULL(dt.ori_wo,'none') AS linker,jv.d_Date AS schedule_d, jv.d_Reschdt AS dtresch,a.v_stest,a.v_ptest,a.v_AcceptedBy,r.v_ref_status", false);
          ...
          $this->db->select("g.V_Asset_name, e.v_location_name, r.v_location_code, r.V_hospitalcode, r.closedby, r.D_date, r.D_time, r.V_Request_no, r.V_Asset_no, r.V_summary AS ReqSummary, r.V_User_dept_code, r.V_requestor, r.V_request_status, r.v_closeddate, r.v_closedtime, w.V_Wrn_end_code, a.v_summary, g.v_tag_no, d.v_UserDeptDesc, CASE WHEN r.V_request_status = 'C' AND r.v_closeddate >= '".$year."-".$month."-08 23:59:59' AND r.v_closeddate < '".$this->dater(2,$month,$year)." 23:59:59' THEN DATEDIFF(r.v_closeddate, '".$year."-".$month."-09 23:59:59')+1 WHEN r.V_request_status = 'C' AND r.v_closeddate < DATE_ADD('".$year."-".$month."-08 23:59:59', INTERVAL 1 MONTH) THEN DATEDIFF(r.v_closeddate, r.D_date)+1 WHEN r.V_request_status <> 'C' AND DAY(LAST_DAY(".$this->db->escape($year."-".$month."-01").")) > DATEDIFF(now(), r.D_date) THEN DATEDIFF( now(),r.D_date)+1 ELSE DAY(LAST_DAY(".$this->db->escape($year."-".$month."-01").")) END AS DiffDate,r.V_request_type,g.v_asset_grp,jr.d_Date,jr.v_Time,jr.v_Personal1,jr.v_ActionTaken,g.V_Asset_WG_code, IFNULL(dt.ori_wo,'none') AS linker, jv.d_Date AS schedule_d, jv.d_Reschdt AS dtresch,a.v_stest,a.v_ptest,a.v_AcceptedBy,r.v_ref_status", false);
			...
			if ($fon == "") {
      if ($reqtype == 'A2' && $pilih == 'C'){
  			//$this->db->where('jv.d_date >=',$this->dater(1,$month,$year));
  			//$this->db->where('jv.d_date <=',$this->dater(2,$month,$year) . ' 23:59:59');
  		}
			$this->db->where('r.v_request_status <> ', $pilih);
			} else {
			$this->db->where('r.v_request_status <> ', $pilih);
			...
			if ($reqtype <> 'AP'){
			$this->db->where('r.V_request_type !=','AP');
			}
			if ($reqtype <> ''){
		...
		/* $this->db->where('r.d_date >=', $this->dater(1,$month,$year));
			$this->db->where('r.d_date <=', $this->dater(2,$month,$year).'  23:59:59'); */
			//$this->db->where("(`r`.`d_date` >= '".$this->dater(1,$month,$year)."' AND `r`.`d_date` <= '".$this->dater(2,$month,$year).'  23:59:59'."'
			//OR `jv`.`d_reschdt` >= '".$this->dater(1,$month,$year)."' AND `jv`.`d_reschdt` <= '".$this->dater(2,$month,$year).'  23:59:59'."')", NULL, FALSE);

      if ($reqtype == 'A2'){

          //echo "nilaik : ".$pilih.", nilairesch : ".$resch;
          if (($pilih=="fbfb") && ($resch=="")) {
            $this->db->where("(`r`.`d_date` >= '".$this->dater(1,$month,$year)."' AND `r`.`d_date` <= '".$this->dater(2,$month,$year).'  23:59:59'."')", NULL, FALSE);
          } elseif ($pilih == 'C') {
            $this->db->where(" (IFNULL(`r`.`v_closeddate`,IFNULL(`jv`.`d_reschdt`,IFNULL(r.v_respondate,r.d_date))) >= '".$this->dater(1,$month,$year)."' AND IFNULL(`r`.`v_closeddate`,IFNULL(`jv`.`d_reschdt`,IFNULL(r.v_respondate,r.d_date))) <= '".$this->dater(2,$month,$year).'  23:59:59'."')", NULL, FALSE);
          }else {
            $this->db->where("(`r`.`d_date` >= '".$this->dater(1,$month,$year)."' AND `r`.`d_date` <= '".$this->dater(2,$month,$year).'  23:59:59'."'
      			OR IFNULL(`r`.`v_closeddate`,IFNULL(`jv`.`d_reschdt`,`r`.`v_respondate`)) >= '".$this->dater(1,$month,$year)."' AND IFNULL(`r`.`v_closeddate`,IFNULL(`jv`.`d_reschdt`,`r`.`v_respondate`)) <= '".$this->dater(2,$month,$year).'  23:59:59'."')", NULL, FALSE);
          }
          ...
          if ($reqtype == 'A2'){
            $this->db->where('jv.n_visit', '1');
				...
				//echo "test";
        if ($reqtype == 'A2'){
			    $this->db->where('IFNULL(r.v_closeddate,IFNULL(jv.d_reschdt, r.v_respondate)) >=', $this->dater(2,$month,$year));
        } else {
          $this->db->where('jv.d_reschdt >=', $this->dater(2,$month,$year));
        }
				$this->db->group_by('jv.v_WrkOrdNo,jv.n_Visit');
			}else{
			//$this->db->group_by('jv.v_WrkOrdNo');
      //$this->db->group_by('COALESCE(jv.v_WrkOrdNo,r.V_Request_no)',false);
      $this->db->group_by('COALESCE(jv.v_WrkOrdNo,r.V_Request_no)');
			}
...
ORDER BY s.d_DueDt, s.v_WrkOrdNo
		...
		*/
			...
			$this->db->where('s.v_servicecode', $this->session->userdata('usersess'));
			$this->db->where('s.V_request_type <> ', 'AP');
			$this->db->where('s.v_actionflag <> ', 'D');
			//$this->db->where('a.v_actionflag <> ', 'D');
...
ORDER BY a.V_Asset_name
		*/
		...
		function rpt_alr($month,$year,$grpsel,$dept,$assetgrp){
		/*
		SELECT DISTINCT
...
ORDER BY a.V_Asset_no
		*/
			...
			if ($assetgrp <> ''){
				$this->db->where("a.V_Equip_code", $assetgrp);
			}
			//$this->db->group_by('a.V_Equip_code, m.new_asset_type, a.V_Asset_name');
...
return $query->result();
		...
		function stock_asset($searchitem=""){
		  $this->db->distinct();
      //$this->db->select('Dept AS Hosp_code,CategoryID AS Qty,ItemCode,REPLACE(REPLACE(ItemName, CHAR(10), ""), CHAR(13), "") AS ItemName',FALSE);
      //$this->db->from('tbl_invitem');
			...
			$this->db->limit(500);
			if ($searchitem != "") {
			$this->db->where("ItemCode",$searchitem)->or_like("ItemName",$searchitem);}
    ...
    function stock_assetx($searchitem=""){
		  $this->db->distinct();
      $this->db->select('Dept AS Hosp_code,CategoryID AS Qty,ItemCode,REPLACE(REPLACE(ItemName, CHAR(10), ""), CHAR(13), "") AS ItemName',FALSE);
      $this->db->from('tbl_invitem');
			////$this->db->select('a.Hosp_code,a.Qty,b.ItemCode,REPLACE(REPLACE(b.ItemName, CHAR(10), ""), CHAR(13), "") AS ItemName',FALSE);
      ...
      if ($reqtype <> 'AP'){
			   $this->db->where('sr.V_request_type !=','AP');
			}
			if ($reqtype <> ''){
				 if ($reqtype == 'F') {
				 //$this->db->like('sr.V_summary', 'floor');
		...
		function sumlicsat($month,$year)
		{
			...
			original del*/
//      if ((substr($this->session->userdata('v_UserName'),0,4)=="iium") || (substr($this->session->userdata('v_UserName'),0,4)=="pmsb")) {
//			$this->db->select("COUNT(*) AS total, COUNT(*) AS notlicsat, SUM(CASE WHEN A.v_ExpiryDate < now() THEN 1 ELSE 0 END) AS licsat ");
...
function sumsat($month,$year,$grpsel)
		...
		function rpt_rsls($month,$year, $stat = "apo2",$expiring){
			...
			//$this->db->select("A.v_CertificateNo, A.v_ServiceCode, A.v_AgencyCode, A.v_registrationno, A.v_LicenseCategoryCode, B.v_LicenceCategoryDesc, A.v_IdentificationType, A.v_Identification, A.v_RegistrationNo, A.v_StartDate, A.v_ExpiryDate, A.v_GradeID, A.v_Remarks, A.v_hospitalcode, A.v_key, A.CMIS_Action_Flag, A.d_timestamp,A.id,i.file_name, IFNULL(C.V_Location_code,l.v_Location_code) as V_Location_code ,l.v_Location_Name",FALSE);
      $this->db->select("A.v_CertificateNo, A.v_ServiceCode, A.v_AgencyCode, A.v_registrationno, A.v_LicenseCategoryCode, B.v_LicenceCategoryDesc, A.v_IdentificationType, A.v_Identification, A.v_RegistrationNo, A.v_StartDate, A.v_ExpiryDate, A.v_GradeID, A.v_Remarks, A.v_hospitalcode, A.v_key, A.CMIS_Action_Flag, A.d_timestamp,A.id,i.file_name,IFNULL(l.v_Location_code,(SELECT v_Location_code FROM pmis2_egm_assetlocation WHERE V_Location_code = A.v_TesterName OR V_Location_code = A.v_Remarks  LIMIT 1)) as V_Location_code,IFNULL(l.v_Location_Name,(SELECT v_Location_Name FROM pmis2_egm_assetlocation WHERE V_Location_code = A.v_TesterName OR V_Location_code = A.v_Remarks LIMIT 1)) as v_Location_Name",FALSE);
    ...
    GROUP BY `a`.`v_CertificateNo` , `a`.`v_RegistrationNo` , `a`.`v_LicenseCategoryCode`)`g`",'concat(concat(A.v_CertificateNo,A.v_RegistrationNo),A.v_ExpiryDate) = concat(concat(g.v_CertificateNo,g.v_RegistrationNo),g.v_ExpiryDate)');
			...
			$this->db->join('pmis2_egm_assetlocation l','C.V_Location_code = l.V_location_code OR `A`.`v_TesterName` = `l`.`V_location_code` AND l.V_Actionflag <> "D" ','left outer');
...
function rpt_sdwo($wono,$wotype){
	if ($wotype == 'Request'){
		$this->db->select('r.V_Request_no AS V_Request_no,r.D_date AS D_date,r.V_User_dept_code,d.v_UserDeptDesc,r.V_Asset_no AS V_Asset_no,a.V_Tag_no,r.V_summary AS V_summary,r.V_request_type AS V_request_type,r.V_request_status AS V_request_status,r.v_respondate AS v_respondate,b.v_summary as aWrkdone');
		$this->db->from('pmis2_egm_service_request r');
		...
		$this->db->join('pmis2_egm_jobdonedet b','r.V_Request_no=b.v_Wrkordno','left outer');
	...
	elseif($wotype == 'PPM'){
		$this->db->select('r.v_WrkOrdNo AS V_Request_no,r.d_StartDt AS D_date,a.V_User_Dept_code,d.v_UserDeptDesc,r.v_Asset_no AS V_Asset_no,a.V_Tag_no,r.v_Remarks AS V_summary,r.v_jobtype AS V_request_type,r.v_Wrkordstatus AS V_request_status,r.v_respondate AS v_respondate,b.v_summary as aWrkdone');
		$this->db->from('pmis2_egm_schconfirmmon r');
		$this->db->join('pmis2_egm_assetregistration a','r.V_Asset_no = a.V_Asset_no AND a.V_Actionflag <> "D"','left outer');
	    ...
	    $this->db->join('pmis2_egm_jobdonedet b','r.v_WrkOrdNo=b.v_Wrkordno','left outer');
...
function reccompday($date)
{
  ...
  $this->db->where('DATE(r.v_closeddate)',$date);
...
function vendorlist($code){
  ...
  $this->db->select('v.Id as v_id,v.actual_vendor,v.Item_Code,v.Vendor,i.VENDOR_NAME,v.Vendor_Item_Code,v.List_Price,i.Id as vi_id');
	$this->db->from('tbl_vendor v');
	//$this->db->join('tbl_vendor_info i','v.Vendor = VENDOR_CODE');
	$this->db->join('tbl_vendor_info i','v.Vendor = VENDOR_CODE','left');
...
function sumrq_y($month,$year,$reqtype,$grpsel,$bystak="")
{
	...
	$this->db->where("month(d_date) > ",$month-1);
	//$this->db->where("year(d_date) = ",$year);
	if ($reqtype <> ''){
		 if ($reqtype == 'F') {
		 //$this->db->like('sr.V_summary', 'floor');
...
ORDER BY r.D_date, r.D_time
     ...
     $this->db->where('r.V_request_type',$reqtype);
			...
			//$this->db->where('YEAR(r.D_date) ', $year);
			$this->db->where('MONTH(r.D_date) > ', $month-1);
			//else{
	...
	function fdrepdet1($date,$reptype,$v){
      ...
      $this->db->select("r.V_Request_no, e.v_location_name, r.v_location_code, r.V_hospitalcode, r.closedby, r.D_date, r.D_time, r.V_Asset_no, r.V_summary AS ReqSummary, r.V_User_dept_code, r.V_requestor, r.V_request_status, r.v_closeddate, r.v_closedtime, w.V_Wrn_end_code, a.v_summary, g.v_tag_no, d.v_UserDeptDesc, CASE WHEN r.V_request_status = 'C' AND r.v_closeddate <= DATE_ADD('".$year."-".$month."-08 23:59:59', INTERVAL 1 MONTH) THEN DATEDIFF(r.v_closeddate, r.D_date)+1 WHEN r.V_request_status <> 'C' AND DAY(LAST_DAY(DATE_FORMAT('2018-09-19','%Y-%m-08'))) > DATEDIFF(now(), r.D_date) THEN DATEDIFF( now(),r.D_date)+1 ELSE ( CASE WHEN  r.V_request_status <> 'C' AND MONTH(NOW()) = MONTH('".$date."') THEN  ABS(DATEDIFF('".$date."','".$year."-".$month."-09 23:59:59')) ELSE DAY(LAST_DAY(DATE_FORMAT('".$date."','%Y-%m-01'))) END ) END AS DiffDate,r.V_request_type,g.v_asset_grp,jr.d_Date,jr.v_Time,jr.v_Personal1,jr.v_ActionTaken,g.V_Asset_WG_code, IFNULL(dt.ori_wo,'none') AS linker,a.v_AcceptedBy", false);
			$this->db->from('pmis2_egm_service_request r');
			$this->db->join('pmis2_egm_assetregistration g','r.v_Asset_no = g.V_Asset_no AND r.v_HospitalCode = g.V_Hospitalcode AND g.V_Actionflag <> "D"', 'left outer');
			$this->db->join('pmis2_egm_assetreg_general w','r.V_Asset_no = w.V_Asset_no AND r.V_hospitalcode = w.V_Hospital_code', 'left outer');
			$this->db->join('pmis2_egm_jobdonedet a',"a.v_Wrkordno = r.V_Request_no AND a.v_HospitalCode = r.V_hospitalcode AND a.v_Actionflag <> 'D'", 'left outer');
        ...
        $this->db->where('DATE(r.v_closeddate)',$date);
				...
				$this->db->where('r.v_request_status <>','C');
				$this->db->where("TIMESTAMPDIFF(MONTH, CASE WHEN r.d_date BETWEEN concat(concat(year(r.d_date),'-'),concat(month(r.d_date)),'-08 23:59:59') AND DATE_ADD(concat(concat(year(r.d_date),'-'),concat(month(r.d_date)),'-09 23:59:59'), INTERVAL 1 MONTH) THEN concat(concat(year(r.d_date),'-'),concat(month(r.d_date)),'-08 23:59:59') ELSE DATE_SUB(concat(concat(year(r.d_date),'-'),concat(month(r.d_date)),'-08 23:59:59'), INTERVAL 1 MONTH) end,  DATE_ADD(concat(concat(year(now()),'-'),concat(month(now())),'-09 00:00:00'), INTERVAL 1 MONTH)) > ","1", false);
				$this->db->where('r.d_date <=', $this->dater(2,$month,$year).'  23:59:59');
			}
			$this->db->where('r.V_servicecode', $this->session->userdata('usersess'));
			$this->db->where('r.V_actionflag <> ', 'D');
			$this->db->where('r.V_hospitalcode',$this->session->userdata('hosp_code'));
			$this->db->order_by('DATE(r.D_date)','ASC');
			$this->db->group_by('r.V_Request_no');
...
/*
  ...
  */
  	...
  	function mrinlist($month,$year,$type,$kelas, $search){
  		...
  		$inter = (int)$month;
  		//$this->db->select('m.*,IFNULL(s.V_Asset_no,p.v_Asset_no) AS V_Asset_no,st.Status, IFNULL(IFNULL(IFNULL(ApprCommentsxx,ApprCommentsx),ApprComments),Comments) AS Commentsx',FALSE);
      $this->db->select('m.*,IFNULL(s.V_Asset_no,p.v_Asset_no) AS V_Asset_no,st.Status, IFNULL(IFNULL(IFNULL(ApprCommentsxx,ApprCommentsx),ApprComments),Comments) AS Commentsx,IFNULL(s.D_date,p.d_StartDt) as wodate,(SELECT V_Tag_no FROM pmis2_egm_assetregistration WHERE V_Asset_no=s.V_Asset_no OR V_Asset_no=p.V_Asset_no LIMIT 1) as Astag, sa.v_username',FALSE);
  		$this->db->from('tbl_materialreq m');
  		$this->db->join('pmis2_egm_service_request s','m.WorkOfOrder = s.V_Request_no AND s.V_actionflag <> "D"','left outer');
  		$this->db->join('pmis2_egm_schconfirmmon p','m.WorkOfOrder = p.v_WrkOrdNo AND p.v_Actionflag <> "D"','left outer');
  		$this->db->join('tbl_status st','m.ApprStatusID = st.StatusID');
  		$this->db->join('tbl_user u','m.Requestuserid = u.userid');
  		$this->db->join('pmis2_sa_user sa','u.login = sa.v_userid');
  		//$this->db->where('MONTH(DATE(m.DateCreated))',$inter);
  		//$this->db->where('YEAR(DATE(m.DateCreated))',$year);
  		$this->db->where('service_code',$this->session->userdata('usersess'));
...
function user_class($username){
	...
	function rpt_alr_bes($month,$year,$grpsel,$dept,$assetgrp){
		/*
		SELECT DISTINCT
                      a.V_Hospitalcode, a.V_Tag_no, e.Asset_Type, e.Type_Desc, a.V_Asset_no, a.V_Equip_code, f.v_Equip_Desc, c.v_AssetStatus, c.v_AssetVStatus,
                      CONVERT(varchar, c.d_RefDate, 106) AS BER_DATE, c.v_AssetCondition, YEAR(GETDATE()) - YEAR(b.D_commission) AS Age, b.V_PO_no,
...
ORDER BY a.V_Asset_no
    ...
    $this->db->join('pmis2_egm_accesories i',"a.V_Asset_no = i.v_assetno AND a.V_Hospitalcode = i.v_hospitalcode AND i.v_actionflag <> 'D'",'left outer');
	  $this->db->join('pmis2_egm_maintaincat mt','mt.id_cat=a.V_GEN_status AND mt.v_service="BES"','left outer');
    $this->db->where('a.V_service_code', $this->session->userdata('usersess'));
    $this->db->where('a.V_Actionflag != ', 'D');
    $this->db->where('b.V_ActionFlag != ', 'D');
    $this->db->where('c.V_ActionFlag != ', 'D');
...
function sumrq_a2($month,$year,$reqtype,$grpsel,$bystak="")
{
	...
	$this->db->select("COUNT(*) as total,SUM(CASE WHEN sr.v_request_status <> 'C' THEN 1 ELSE 0 END) AS notcomp,SUM(CASE WHEN sr.v_request_status = 'C' THEN 1 ELSE 0 END) AS comp,SUM(CASE WHEN (TIMESTAMPDIFF(MINUTE,sr.d_date,IFNULL(sr.v_respondate,NOW())) <= $dn AND sr.V_priority_code = 'Normal') OR (TIMESTAMPDIFF(MINUTE,sr.d_date,IFNULL(sr.v_respondate,NOW())) <= $de AND sr.V_priority_code = 'Emergency') THEN 1 ELSE 0 END) AS resp,SUM(CASE WHEN (TIMESTAMPDIFF(MINUTE,sr.d_date,IFNULL(sr.v_respondate,NOW())) > $dn AND sr.V_priority_code = 'Normal') OR (TIMESTAMPDIFF(MINUTE,sr.d_date,IFNULL(sr.v_respondate,NOW())) > $de AND sr.V_priority_code = 'Emergency') THEN 1 ELSE 0 END) AS resplate,SUM(kira) as resch");

	$this->db->from('pmis2_egm_service_request sr');
	$this->db->join('pmis2_egm_assetregistration a','sr.V_Asset_no = a.V_Asset_no AND sr.V_hospitalcode = a.V_Hospitalcode AND a.V_Actionflag <> "D"','left outer');
	$this->db->join("(SELECT v_WrkOrdNo, SUM(CASE WHEN d_reschdt is not NULL AND d_Date < '".$this->dater(1,$month,$year)."' THEN 1 ELSE 0 END) AS kira FROM pmis2_emg_jobvisit1
                      GROUP BY v_WrkOrdNo)`jv`",'sr.V_Request_no=jv.v_WrkOrdNo','left outer');
	$this->db->where('sr.v_Actionflag <> ','D');
	//$this->db->where('a.V_Actionflag <> ','D');
	$this->db->where('sr.v_ServiceCode = ',$this->session->userdata('usersess'));
	if ($grpsel <> ''){
...
function sumrq_a2($month,$year,$reqtype,$grpsel,$bystak="")
{
	...
	//$this->db->select("COUNT(*) as total,SUM(CASE WHEN sr.v_request_status <> 'C' AND IFNULL(jv.D_date,jv.d_reschdt) >= '".$this->dater(1,$month,$year)."' AND IFNULL(jv.D_date,jv.d_reschdt) <= '".$this->dater(2,$month,$year)."  23:59:59' THEN 1 ELSE 0 END) AS notcomp,SUM(CASE WHEN sr.v_request_status = 'C' THEN 1 ELSE 0 END) AS comp,
	//SUM(CASE WHEN (TIMESTAMPDIFF(MINUTE,sr.d_date,IFNULL(sr.v_respondate,NOW())) <= $dn AND sr.V_priority_code = 'Normal') OR (TIMESTAMPDIFF(MINUTE,sr.d_date,
	//IFNULL(sr.v_respondate,NOW())) <= $de AND sr.V_priority_code = 'Emergency') THEN 1 ELSE 0 END) AS resp,
	//SUM(CASE WHEN (TIMESTAMPDIFF(MINUTE,sr.d_date,IFNULL(sr.v_respondate,NOW())) > $dn AND sr.V_priority_code = 'Normal') OR (TIMESTAMPDIFF(MINUTE,sr.d_date,
	//IFNULL(sr.v_respondate,NOW())) > $de AND sr.V_priority_code = 'Emergency') THEN 1 ELSE 0 END) AS resplate,
	//SUM(kira) AS resch");

  $this->db->select("SUM(CASE WHEN `sr`.`d_date` >= '".$this->dater(1,$month,$year)."' AND `sr`.`d_date` <= '".$this->dater(2,$month,$year)."  23:59:59' THEN 1 ELSE 0 END) as total,SUM(CASE WHEN sr.v_request_status <> 'C' AND IFNULL(sr.v_closeddate,IFNULL(jv.d_reschdt, IFNULL(sr.v_respondate,sr.d_date))) >= '".$this->dater(1,$month,$year)."' AND IFNULL(sr.v_closeddate,IFNULL(jv.d_reschdt, IFNULL(sr.v_respondate,sr.d_date))) <= '".$this->dater(2,$month,$year)."  23:59:59' THEN 1 ELSE 0 END) AS notcomp,SUM(CASE WHEN sr.v_request_status = 'C' AND IFNULL(sr.v_closeddate,IFNULL(jv.d_reschdt, sr.v_respondate)) >= '".$this->dater(1,$month,$year)."' AND IFNULL(sr.v_closeddate,IFNULL(jv.d_reschdt, sr.v_respondate)) <= '".$this->dater(2,$month,$year)."  23:59:59' THEN 1 ELSE 0 END) AS comp,
	SUM(CASE WHEN (TIMESTAMPDIFF(MINUTE,sr.d_date,IFNULL(sr.v_respondate,NOW())) <= $dn AND sr.V_priority_code = 'Normal') OR (TIMESTAMPDIFF(MINUTE,sr.d_date,
	IFNULL(sr.v_respondate,NOW())) <= $de AND sr.V_priority_code = 'Emergency') THEN 1 ELSE 0 END) AS resp,
	SUM(CASE WHEN (TIMESTAMPDIFF(MINUTE,sr.d_date,IFNULL(sr.v_respondate,NOW())) > $dn AND sr.V_priority_code = 'Normal') OR (TIMESTAMPDIFF(MINUTE,sr.d_date,
	IFNULL(sr.v_respondate,NOW())) > $de AND sr.V_priority_code = 'Emergency') THEN 1 ELSE 0 END) AS resplate,
	SUM(CASE WHEN kira > 0 THEN 1 ELSE 0 END) AS resch");

	$this->db->from('pmis2_egm_service_request sr');
	$this->db->join('pmis2_egm_assetregistration a','sr.V_Asset_no = a.V_Asset_no AND sr.V_hospitalcode = a.V_Hospitalcode AND a.V_Actionflag <> "D"','left outer');
// 	$this->db->join("(SELECT v_WrkOrdNo, SUM(CASE WHEN  d_Date < '".$this->dater(1,$month,$year)."' THEN 1 ELSE 0 END) AS kira FROM pmis2_emg_jobvisit1
//                      GROUP BY v_WrkOrdNo)`jv`",'sr.V_Request_no=jv.v_WrkOrdNo','left outer');

	//$this->db->join("(SELECT a.D_date, b.V_Request_no,a.d_reschdt,SUM(CASE WHEN  b.d_Date < '".$this->dater(1,$month,$year)."' THEN 1 ELSE 0 END) AS kira FROM pmis2_emg_jobvisit1 a
  //  LEFT OUTER JOIN pmis2_egm_service_request b  ON a.v_WrkOrdNo=b.V_Request_no WHERE IFNULL(b.v_closeddate,IFNULL(a.d_reschdt,b.d_Date)) >= '".$this->dater(1,$month,$year)."' AND IFNULL(b.v_closeddate,IFNULL(a.d_reschdt,b.d_Date)) <= '".$this->dater(2,$month,$year)." 23:59:59' AND right(left(b.V_Request_no,5), 2) = 'A2' GROUP BY b.V_Request_no)`jv`",'sr.V_Request_no=jv.V_Request_no','left outer');
  $this->db->join("(SELECT a.D_date, b.V_Request_no,a.d_reschdt,SUM(CASE WHEN  b.d_Date < '".$this->dater(1,$month,$year)."' THEN 1 ELSE 0 END) AS kira FROM pmis2_emg_jobvisit1 a
    LEFT OUTER JOIN pmis2_egm_service_request b  ON a.v_WrkOrdNo=b.V_Request_no WHERE IFNULL(b.v_closeddate,IFNULL(a.d_reschdt,IFNULL(a.d_Date,b.d_Date))) >= '".$this->dater(1,$month,$year)."' AND right(left(b.V_Request_no,5), 2) = 'A2' GROUP BY b.V_Request_no)`jv`",'sr.V_Request_no=jv.V_Request_no','left outer');

	/* $this->db->join('pmis2_emg_jobvisit1 jv','sr.V_Request_no=jv.v_WrkOrdNo','left outer'); */
	$this->db->where('sr.v_Actionflag <> ','D');
	//$this->db->where('a.V_Actionflag <> ','D');
	$this->db->where('sr.v_ServiceCode = ',$this->session->userdata('usersess'));
	if ($grpsel <> ''){
	...
	$this->db->where("(`sr`.`d_date` >= '".$this->dater(1,$month,$year)."' AND `sr`.`d_date` <= '".$this->dater(2,$month,$year).'  23:59:59'."'
	OR IFNULL(`sr`.`v_closeddate`,IFNULL(jv.d_reschdt,`sr`.`v_respondate`)) >= '".$this->dater(1,$month,$year)."' AND IFNULL(`sr`.`v_closeddate`,IFNULL(jv.d_reschdt,`sr`.`v_respondate`)) <= '".$this->dater(2,$month,$year).'  23:59:59'."')", NULL, FALSE);
                if (!function_exists('toArray')) {
	function toArray($obj)
	{
     $obj = (array) $obj;//cast to array, optional
...
echo $this->db->last_query();
    ...
    function s_item_detail($limit,$start, $search=''){
  	...
  	function assetEquip(){
  		...
  		$this->db->where("v_hospitalcode", $this->session->userdata("hosp_code"));
  		$this->db->where("v_ServiceCode", $this->session->userdata('usersess'));
  		$this->db->group_by("v_Equip_Code");

  		$query = $this->db->get("pmis2_sa_equip_code");
  		// echo "<pre>".$this->db->last_query();
  		return $query->result();
  	}
  	function a2_reschout($month,$year,$reqtype,$grpsel,$bystak = ""){
     /*  echo $bystak;
  	exit(); */
  		...
  		if ($bystak == "IIUM C") {
  		    ...
  		    //$this->db->select("SUM(CASE WHEN sc.d_reschdt is not NULL AND sc.v_wrkordstatus = 'AR' AND (IFNULL(sc.d_reschdt, d_DueDt) > now()) THEN 1 ELSE 0 END) AS reschout",FALSE);
  				//$this->db->select("SUM(CASE WHEN jv.d_reschdt is not NULL AND jv.d_reschdt >= '".$this->dater(2,$month,$year)."' THEN 1 ELSE 0 END) AS reschout",FALSE);
          $this->db->select("SUM(CASE WHEN  IFNULL(sr.v_closeddate,IFNULL(jv.d_reschdt, sr.v_respondate)) >= '".$this->dater(2,$month,$year)."' THEN 1 ELSE 0 END) AS reschout",FALSE);
  			$this->db->from('pmis2_egm_service_request sr');
  			$this->db->join('pmis2_egm_assetregistration a','sr.V_Asset_no = a.V_Asset_no AND sr.V_hospitalcode = a.V_Hospitalcode AND a.V_Actionflag <> "D"'.$bystak,'left outer');
  		    $this->db->join('pmis2_emg_jobvisit1 jv','sr.V_Request_no = jv.v_WrkOrdNo','left outer');
  		/* 	$this->db->join("(SELECT v_WrkOrdNo, SUM(CASE WHEN d_reschdt is not NULL AND d_reschdt < '".$this->dater(2,$month,$year)."' THEN 1 ELSE 0 END) AS reschout FROM pmis2_emg_jobvisit1
  GROUP BY v_WrkOrdNo)`jv`",'sr.V_Request_no=jv.v_WrkOrdNo'); */
  			$this->db->where('sr.v_Actionflag <> ','D');
  			$this->db->where('a.v_Actionflag <> ','D');
  			$this->db->where('sr.v_ServiceCode = ',$this->session->userdata('usersess'));
      ...
      function wo_no_mrin($year, $month){
        ...
        $this->db->where("wo.V_hospitalcode", $this->session->userdata("hosp_code"));
...
/* 	echo $x;
    ...
    $this->db->select("sc.v_WrkOrdNo,sc.d_StartDt,jr.v_ActionTaken,d.v_UserDeptDesc,g.v_asset_grp, e.v_location_name, g.V_Location_code,sc.v_Wrkordstatus, sc.v_HospitalCode, sc.closedby, sc.d_DueDt, sc.v_Asset_no, sc.v_Remarks AS ReqSummary,g.v_tag_no,sc.v_closeddate,sc.v_closedtime,
    CASE WHEN sc.v_Wrkordstatus = 'C' AND sc.v_closeddate <= DATE_ADD('".$year."-".$month."-08 23:59:59', INTERVAL 1 MONTH) THEN  CASE WHEN (DATEDIFF(sc.v_closeddate, sc.d_DueDt) + 1) < 0 THEN 0 ELSE (DATEDIFF(sc.v_closeddate, sc.d_DueDt) + 1) END WHEN sc.v_Wrkordstatus <> 'C'
    AND DAY(LAST_DAY(".$this->db->escape($year."-".$month."-01").")) > DATEDIFF(now(), sc.d_DueDt) THEN CASE WHEN (DATEDIFF(NOW(), sc.d_DueDt) + 1) < 0 THEN 0 ELSE (DATEDIFF(NOW(), sc.d_DueDt) + 1) END ELSE DAY(LAST_DAY(".$this->db->escape($year."-".$month."-01").")) END AS DiffDate,
    IFNULL(dt.ori_wo,'none') AS linker", false);
			$this->db->from('pmis2_egm_schconfirmmon sc');
			$this->db->join('pmis2_egm_assetregistration g','sc.v_Asset_no = g.V_Asset_no AND sc.v_HospitalCode = g.V_Hospitalcode AND g.v_Actionflag <> "D"', 'left outer');
			$this->db->join('pmis2_egm_assetreg_general w','sc.v_Asset_no = w.V_Asset_no AND sc.v_HospitalCode = w.V_Hospital_code', 'left outer');
			$this->db->join('pmis2_egm_jobdonedet a',"a.v_Wrkordno = sc.v_WrkOrdNo AND a.v_HospitalCode = sc.v_HospitalCode AND a.v_Actionflag <> 'D'", 'left outer');
			$this->db->join('pmis2_sa_userdept d','g.V_User_Dept_code = d.v_UserDeptCode','left');
			$this->db->join('pmis2_egm_assetlocation e','g.V_Location_code = e.v_location_code','left outer');
			$this->db->join('pmis2_emg_jobresponse jr',"sc.v_WrkOrdNo = jr.v_WrkOrdNo",'left outer');
			$this->db->join('pmis2_egm_sharedowntime dt',"sc.v_WrkOrdNo = dt.ori_wo",'left outer');

		 if ($x == 1){
				$this->db->where('sc.v_WrkOrdNo  LIKE','%PP%');
				$this->db->where('IFNULL(sc.d_reschdt,sc.d_DueDt) >=', $this->dater(1,$month,$year));
	            $this->db->where('IFNULL(sc.d_reschdt,sc.d_DueDt) <=', $this->dater(2,$month,$year));
			}
		elseif($x == 2){
				$this->db->where('sc.v_WrkOrdNo  LIKE','%PP%');
				$this->db->where('LEFT(sc.v_Wrkordstatus, 1) =','C');
				$this->db->where('IFNULL(sc.d_reschdt,sc.d_DueDt) >=', $this->dater(1,$month,$year));
	            $this->db->where('IFNULL(sc.d_reschdt,sc.d_DueDt) <=', $this->dater(2,$month,$year));
		    }
	   elseif($x == 3){
	   ...
	   elseif($x == 4){
				$this->db->where('sc.v_WrkOrdNo  LIKE','%RI%');
				$this->db->where('IFNULL(sc.d_reschdt,sc.d_DueDt) >=', $this->dater(1,$month,$year));
	            $this->db->where('IFNULL(sc.d_reschdt,sc.d_DueDt) <=', $this->dater(2,$month,$year));
		    }
	   elseif($x == 5){
				$this->db->where('sc.v_WrkOrdNo  LIKE','%RI%');
				$this->db->where('sc.v_Wrkordstatus','C');
				$this->db->where('IFNULL(sc.d_reschdt,sc.d_DueDt) >=', $this->dater(1,$month,$year));
	            $this->db->where('IFNULL(sc.d_reschdt,sc.d_DueDt) <=', $this->dater(2,$month,$year));
		    }
	   elseif($x == 6){
...
function get_release_note($maklumat){
  ...
  return $result;
      ...
      function area_list(){
        $query = $this->db->select("*")->from("pmis2_sa_hospital")->where("v_Actionflag <> 'D'")->get()->result_array();
        return $query;
      }


      function rl_mrin($hosp,$y){
        $this->db->select('a.ItemCode,a.ItemName,b.MIRNcode,b.QtyReq,d.qty as qstore');
      ...
      if($hosp=='COE'){
      $this->db->where('c.Apprstatusidxx = 4 AND d.qty > 0');
      $this->db->order_by('b.ItemCode');
      }else{
      $this->db->where("c.Apprstatusidxx = 4 AND (b.MIRNcode LIKE '%".$hosp."%') AND d.qty > 0");
      ...
      return $query->result();
        ...
        function rcmvolu($month,$year,$req)
        {
         ...
         $this->db->order_by("r.d_date, g.v_tag_no");
            ...
            function rpt_voluccc($month,$year,$pilih='',$reqtype,$broughtfwd,$grpsel,$bystak="",$tag,$cm,$limab,$a_bfwd="",$fon="",$resch=""){
        /*      echo $bystak;
        	 exit(); */
                    if ($bystak == "IIUM C") {
        			$this->db->where('left(g.v_tag_no,6)', 'IIUM C');
        			...
        			$this->db->distinct();
                if ($broughtfwd == ''){
                $this->db->select("g.V_Asset_name, e.v_location_name, r.v_location_code, r.V_hospitalcode, r.closedby, r.D_date, r.D_time, r.V_Request_no, r.V_Asset_no, r.V_summary AS ReqSummary, r.V_User_dept_code, r.V_requestor, r.V_request_status, r.v_closeddate, r.v_closedtime, w.V_Wrn_end_code, a.v_summary, g.v_tag_no, d.v_UserDeptDesc, DATEDIFF(IFNULL(r.v_closeddate,'".$this->dater(3,$month,$year)."'),r.D_date) + 1 AS DiffDate,r.V_request_type,g.v_asset_grp,jr.d_Date,jr.v_Time,jr.v_Personal1, IFNULL((select v_ActionTaken from pmis2_emg_jobvisit1 where v_WrkOrdNo = r.V_Request_no order by n_Visit desc limit 1),r.v_summary) as v_ActionTaken,g.V_Asset_WG_code, IFNULL(dt.ori_wo,'none') AS linker,jv.d_Date AS schedule_d, jv.d_Reschdt AS dtresch,a.v_stest,a.v_ptest,a.v_AcceptedBy,r.v_ref_status, m.DocReferenceNo,k.MIRN_No,k.PO_No,m.DateCreated,n.PO_Date", false);
                  ...
                  $this->db->select("g.V_Asset_name, e.v_location_name, r.v_location_code, r.V_hospitalcode, r.closedby, r.D_date, r.D_time, r.V_Request_no, r.V_Asset_no, r.V_summary AS ReqSummary, r.V_User_dept_code, r.V_requestor, r.V_request_status, r.v_closeddate, r.v_closedtime, w.V_Wrn_end_code, a.v_summary, g.v_tag_no, d.v_UserDeptDesc, CASE WHEN r.V_request_status = 'C' AND r.v_closeddate >= '".$year."-".$month."-08 23:59:59' AND r.v_closeddate < '".$this->dater(2,$month,$year)." 23:59:59' THEN DATEDIFF(r.v_closeddate, '".$year."-".$month."-09 23:59:59')+1 WHEN r.V_request_status = 'C' AND r.v_closeddate < DATE_ADD('".$year."-".$month."-08 23:59:59', INTERVAL 1 MONTH) THEN DATEDIFF(r.v_closeddate, r.D_date)+1 WHEN r.V_request_status <> 'C' AND DAY(LAST_DAY(".$this->db->escape($year."-".$month."-01").")) > DATEDIFF(now(), r.D_date) THEN DATEDIFF( now(),r.D_date)+1 ELSE DAY(LAST_DAY(".$this->db->escape($year."-".$month."-01").")) END AS DiffDate,r.V_request_type,g.v_asset_grp,jr.d_Date,jr.v_Time,jr.v_Personal1,jr.v_ActionTaken,g.V_Asset_WG_code, IFNULL(dt.ori_wo,'none') AS linker, jv.d_Date AS schedule_d, jv.d_Reschdt AS dtresch,a.v_stest,a.v_ptest,a.v_AcceptedBy,r.v_ref_status,m.DocReferenceNo,k.MIRN_No,k.PO_No,m.DateCreated,n.PO_Date", false);
        			...
        			$this->db->join('pmis2_emg_jobvisit1 jv',"r.V_Request_no = jv.v_WrkOrdNo AND r.v_HospitalCode = jv.v_HospitalCode AND jv.v_Actionflag <> 'D'",'left outer');
        			$this->db->join('tbl_materialreq m','m.WorkOfOrder = r.V_Request_no','left');
        			$this->db->join('tbl_po_mirn k','k.MIRN_No = m.DocReferenceNo','left');
        			$this->db->join('tbl_po n','n.PO_No = k.PO_No','left');
