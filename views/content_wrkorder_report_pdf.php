...
<table class="rport-header">
		<tr>
			...
			<?php }else{ ?>Work Order Listing Details Report<?php }?>- <?=date('F', mktime(0, 0, 0, $month, 10))?> <?=$year?> - <?php echo $this->session->userdata('usersessn');?>  ( <?php if (($this->input->get('req')) and (($this->input->get('grp') == '2') or ($this->input->get('grp') == '3'))){ echo 'Group'.$this->input->get('grp').','.$tulis; } elseif ($this->input->get('req')){echo $tulis; }elseif ($this->input->get('grp') == ''){ echo 'All';}else{ echo 'Group '.$this->input->get('grp');} ?> )</h3></td>
			<?php } ?>
		</tr>
	</table>
	<table class="tftable" border="1" style="text-align:center; font-size:7px;" cellpadding="5" cellspacing="0">
		<tr nobr="true">
			...
			<th <?=$rowspan?> style="width:38px;">Request No</th>
			<th <?=$rowspan?>>Asset No</th>
			<th <?=$rowspan?>>Asset Description</th>
			<th <?=$rowspan?> style="width:38px;">Request Summary</th>
			<th <?=$rowspan?>>ULC</th>
			...
			<th <?=$rowspan?>>Completion Date</th>
			<th <?=$rowspan?>>Completion Time</th>
			<?php } ?>
			<th <?=$rowspan?>>Dept/Loc</th>
			<th <?=$rowspan?>>Mrin No </th>
			<th <?=$rowspan?>>Mrin Date</th>
			<th <?=$rowspan?>>PO No</th>
			<th <?=$rowspan?>>PO Date</th>
